...
Palworld の REST 管理パスワード `PALWORLD_ADMIN_PASSWORD` も初期値は `admin123` です。Web 管理アカウントとは別の設定です。LAN のゲームポートは UDP `8211`、Steam クエリは UDP `27015` です。REST ポート `8212` はコンテナ内部専用です。
...
- Terraria は TShock を使用します。LAN のゲームポートは TCP `7777`、管理ポート `7878` は NAS ローカル専用です。
- Project Zomboid は Build 42 と RCON を使用します。LAN のゲームポートは UDP `16261`–`16263`、RCON TCP `27016` は NAS ローカル専用です。
...
Docker Socket へのアクセスには高いコンテナ管理権限があります。画面から任意のコンテナ名、イメージ、コマンドを入力することはできません。コントローラーは信頼できる LAN 内だけで使用してください。ログイン後の一時セッションは 12 時間有効です。HTTP は認証情報やセッションを暗号化しません。

プレイヤーの IP アドレスは機密情報です。詳細画面は信頼できる LAN 内だけで使用してください。Minecraft で `ONLINE_MODE=FALSE` を設定するとプレイヤー名を偽装できます。
